    ...
    <section id="education" className="relative py-24">
      <div className="section-container relative z-10">
        ...
        <div className="relative mx-auto max-w-4xl">
          <div className="absolute bottom-0 left-5 top-0 w-px -translate-x-1/2 bg-gradient-to-b from-sky-400 via-cyan-300 to-violet-400 sm:left-1/2" />
            ...
            {education.map((item, index) => {
              ...
              return (
                <motion.article
                  ...
                  className="relative grid items-center gap-4 sm:grid-cols-2"
                  initial={{ opacity: 0, y: 30 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true, margin: '-100px' }}
                  transition={{ duration: 0.6 }}
                >
                  <div className="absolute left-5 top-6 z-10 grid h-10 w-10 -translate-x-1/2 place-items-center rounded-full border border-sky-300/40 bg-slate-950 text-sky-300 shadow-glow sm:left-1/2 light:bg-white light:text-sky-700">
                    <GraduationCap size={20} />
                  </div>

                  <div
                    className={`glass-card ml-12 rounded-[2rem] p-6 sm:ml-0 ${
                      isEven ? 'sm:col-start-1 sm:mr-8' : 'sm:col-start-2 sm:ml-8'
                    }`}
                  >
